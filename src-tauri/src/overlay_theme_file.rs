...
pub fn read(app: &AppHandle) -> ThemeFileState {
    let state = match env_override() {
        ...
        EnvOverride::Path(path) => {
            ...
            // create when it is missing (07 §8, amended — see the slice 4
            // review work log for the deviation this records).
            read_candidates(
...
/// apart. 07 §5 treats a dangling symlink as absent, the same as a directory,
/// but — like a directory — worth one `warn!` rather than the silent
/// not-found path: a theming tool's symlink pointing at nothing is a
/// misconfiguration, not the ordinary "no file here yet".
...
fn parse_version(value: Option<&Value>, diagnostics: &mut Vec<ThemeFileDiagnostic>) -> Option<u32> {
    ...
    match value.as_u64() {
        Some(version) if version >= 1 && version <= u32::MAX as u64 => {
...
mod tests {
    ...
    /// 07 §2's inherit-everything example, byte-identical. Frozen: if a schema
    /// change breaks it, add tolerance rather than editing the fixture.
    const EXAMPLE_INHERIT: &str = r##"{ "version": 1 }"##;

    /// 07 §2's explicit spelling of the same thing.
  ...
  "waveform_gap": null
    ...
    /// 07 §2's fully custom theme, byte-identical to 02 §6.
  ...
  "waveform_gap": 2
    ...
    /// 07 §2's theming-tool document: every leniency at once, plus a comment
    /// key and a key from a schema this build does not have.
  ...
  "app_theme": "dark"
    ...
    /// 07 §5: a dangling symlink (its target does not exist) is treated as
    /// absent, the same as a directory — but not silently, or it would be
