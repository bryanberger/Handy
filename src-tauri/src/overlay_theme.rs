...
//!
//! An overlay theme is the set of sixteen optional tokens that decide how the
//! recording overlay's card looks. Every token is optional, and absent means
//! inherit, so the overlay uses Handy's built-in, theme-aware value. A theme
//! that sets nothing reproduces today's overlay exactly.
//!
...
//! and the theme file's state. It is both the return type of
...
impl GlassMaterial {
    /// Declaration order, which is the order the Appearance tab's dropdown
...
impl GlassStyle {
    ...
    /// control and the theme file's documentation list them in.
...
/// second half, what [`liquid_tint`] composes when the token is unset, and it
/// must carry the same number as `GLASS_TINT_INHERIT` in
/// `src/lib/overlayTheme.ts`, where the first half lives. Measured on
/// macOS 26: 0.45 holds the transcript at 5.6 to 9.6:1 across both Glass
/// styles and both app themes, where 0.30 drops it to 4.3:1 under a Light app
/// theme. The same number serves both engines, since the fallback blur was
/// measured at 0.45 too.
#[cfg(any(target_os = "macos", test))]
...
/// Lowest accepted `glass_tint`. Zero is a legitimate value here, unlike
/// `surface_opacity`'s floor. It asks for untinted glass, which is Apple's
/// own look, and the blur behind it still makes the card visible.
...
///
/// The invariant this bound exists for: the widest the control row's centre
/// column can ever be is `9 * WAVEFORM_WIDTH_MAX + 8 * WAVEFORM_GAP_MAX + 8`
...
impl OverlayTheme {
    ...
    /// and the apply layer, so nothing can clamp differently.
    ...
    /// become `None`, meaning inherit, rather than a clamped number. They
    /// cannot be serialized to JSON, so dropping them here is also what keeps
    /// the settings store writable.
...
pub fn merge(file: &OverlayTheme, settings: &OverlayTheme) -> OverlayTheme {
    OverlayTheme {
        ...
        // No `.or(settings.glass_material)` here. The tab has no control
        // for it, so the store cannot be the source.
...
/// hand. The commit path just wrote it; the preview path was handed a draft
/// that never reaches the store. Neither has any reason to read it back.
...
/// overlay shows. It touches the filesystem, so it must only ever be called
/// off the main thread. The overlay show path and the Reload command are its
/// two callers, and both are.
...
/// (`settings::get_settings`). The overlay show path has already read them,
/// to get `overlay_style` and decide whether to show at all, so it passes the
/// tokens it holds instead of paying for a second read on every recording.
...
///
/// Order matters. It emits to the webviews first, because a repaint is the
/// slowest link, and resizes the native window second, because a change to
/// `size_scale` changes how much room the card needs. The resize takes the
/// theme already resolved here, so nothing is resolved twice, and it happens
/// only when something the window is built from actually changed. See
...
/// neither could have changed. The two cannot be separated, because window
/// slack, and so the window size, is what the Material sets to zero under
/// Glass.
...
mod tests {
    ...
    fn the_liquid_tint_alpha_inherits_clamps_and_can_vanish() {
        ...
        // Zero is untinted glass, Apple's own look, rather than a colour with
        // no alpha, so the setter is handed nil. Unlike the surface's 0.30
        // floor, a theme can ask for this one.
    ...
    /// native tint at all, so the theme below, which is what a user who set
    /// Flat to 1.00 and then picked Glass actually has, composes the glassy
    ...
    /// This test writes the bounds as the literals from the token contract
    /// rather than as the module's constants. A mistyped constant must fail
    /// this test rather than redefine what it is checking.
    #[test]
    ...
    /// entirely on clamping being idempotent. If `normalized` moved a value
    /// a second time, a re-sent theme would never compare equal and the skip
    /// would never fire. Worse, an out-of-range value would be stored once
    /// and then "changed" on every later commit.
    #[test]
    ...
    fn normalized_clamps_every_token() {
        ...
        // non-finite float into `null`, so it would pass even without the drop.
        // What matters is that the token is gone, and therefore serializes as
        // an absent value.
        assert_eq!(
    ...
    /// `size_scale`'s are, with the literals from the token table rather than
    ...
    fn glass_material_keys_are_the_serde_spelling() {
        ...
        // The default is the measured pick: the most see-through of the
        // eight, in both app themes.
    ...
    fn merge_prefers_the_file_per_key() {
        ...
        // Merging with an absent file gives the settings back unchanged. That
        // holds for every token the settings can carry, which is all of them
        // but the one below.
    ...
    /// and a Glass request renders Flat while `available` is false. That last
    /// one is exactly the state this build ships in, before the native Glass
    /// module.
    #[test]
    fn resolve_clamps_once_and_downgrades_glass_when_unavailable() {
        ...
        // The file state rides through untouched. It is what draws the
        // tab's lock markers.
    ...
    /// folded into `--s-surface`'s own `color-mix`, and `glass_tint` is
    /// measured rather than declared, so both are pinned to the apply layer's
    ...
    fn overlay_token_inherit_values_match_the_css() {
        ...
        // that they still do. For the tint, it also pins that Rust's copy of
        // the same measured number agrees.
        for entry in [
